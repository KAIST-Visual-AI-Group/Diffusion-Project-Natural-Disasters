...
class VPTree:
    ...
    def __init__(self, points, dist_fn, point_ids):
        self.left = None
        self.right = None
        ...
        self.vp_i = point_ids[0]
        self.vp = points[0]        
        point_ids = np.delete(point_ids, 0, axis=0)
        ...
        assert len(points) == len(point_ids), (
            f"{len(points)} != {len(point_ids)}"
        )

        for point, point_id, distance in zip(points, point_ids, distances):
            if distance >= median:
                ...
                if distance > self.right_max:
                    ...
                    right_point_ids.insert(0, point_id)
                else:
                    right_points.append(point)
                    right_point_ids.append(point_id)
            else:
                ...
                if distance > self.left_max:
                    ...
                    left_point_ids.insert(0, point_id)
                else:
                    left_points.append(point)
                    left_point_ids.append(point_id)
        
        assert len(left_points) + len(right_points) == len(points), (
            f"{len(left_points)} + {len(right_points)} != {len(points)}"
        )
        assert len(left_point_ids) + len(right_point_ids) == len(point_ids), (
            f"{len(left_point_ids)} + {len(right_point_ids)} != {len(point_ids)}"
        )

        if len(left_points) > 0:
            self.left = VPTree(points=left_points, dist_fn=self.dist_fn, point_ids=left_point_ids)

        if len(right_points) > 0:
            self.right = VPTree(points=right_points, dist_fn=self.dist_fn, point_ids=right_point_ids)
    ...
    def get_n_nearest_neighbors(self, query, n_neighbors):
        ...
        while queue:
            ...
            if d < furthest_d or need_neighbors:
                neighbors.append((d, node.vp, node.vp_i))
